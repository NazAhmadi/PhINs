...
class PINNProblem:
    ...
    def residual_arrays(self, trainer_state, apply_net=None, t_collocation=None) -> Dict[str, jnp.ndarray]:
        db = self.data_bundle
        t_col = db.t_collocation if t_collocation is None else t_collocation
        if t_col is None:
            return {}
        ctx_col = self.make_context(trainer_state, t_col, apply_net=apply_net)
        res = self.residual_fn(ctx_col, t_col)
        out = {}
    ...
    def loss_terms(
        ...
        res_arrays = self.residual_arrays(trainer_state, apply_net=apply_net, t_collocation=t_collocation)
        if res_arrays:
